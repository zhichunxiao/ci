    ...
    for (task in tasks) {
        ...
        switch(taskType) {
            ...
            case "jenkins-it-trigger":
                ...
                jobs[taskName] = {
                    def result = common.jenkinsItTrigger(jenkinsITConfig,branch,commitID,taskName,"daily")
            ...
            case "atom-tcms":
                ...
                jobs[taskName] = {
                    def result = common.tcmsTest(tcmsConfig,repo,taskName,"daily")
                    println "result: ${result.getResult()}"
                    task_result_array << ["name": taskName, "type": taskType, "result": result]
                    if (result.getResult() != "SUCCESS") {
                        throw new Exception("${taskName} failed")
            ...
            case "atom-utf":
                ...
                jobs[taskName] = {
                    def result = common.utfTest(utfConfig,repo,taskName,"daily")
                    println "result: ${result.getResult()}"
                    task_result_array << ["name": taskName, "type": taskType, "result": result]
                    if (result.getResult() != "SUCCESS") {
                        throw new Exception("${taskName} failed")
