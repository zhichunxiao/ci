...
def get_sha(repo) {
    ...
    return sh(returnStdout: true, script: "python gethash.py -repo=${repo} -version=${RELEASE_BRANCH} -s=${FILE_SERVER_URL}").trim()
}
...
def release_one(repo,failpoint) {
    ...
    def image = "hub.pingcap.net/qa/${repo}:${IMAGE_TAG}"
    if (failpoint) {
    ...
    if (repo == "br") {
        ...
        def imageLightling = "hub.pingcap.net/qa/tidb-lightning:${IMAGE_TAG}"
